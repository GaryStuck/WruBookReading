...
Axios.interceptors.request.use(
  ...
  error => {
    ...
    Message({
      //  饿了么的消息弹窗组件,类似toast
      showClose: true,
      message: error && error.resultData,
      type: 'error'
    });
    return Promise.reject(error.resultData);
  }
);
...
Axios.interceptors.response.use(
  ...
  error => {
    ...
    // 返回 response 里的错误信息
    let errorInfo = error.resultData ? error.resultData : error;
...
//     if (response) {
//       // 请求已发出，但是不在2xx的范围
//       errorHandle(resultCode, response.data.message);
